...
/// @param res 存放结果
inline void regSimple(string target, const char* sreg, vector<string>& res) {
	smatch sm;
...
/// @param str 输入字符串
/// @return 目标数
inline double strToDob(string str) {
	double
		res = 0,
		digit = 0,
		is正 =
...
inline bool isABC(char a) {
	return (a >= 'a' && a <= 'z') || (a >= 'A' && a <= 'Z');
}

/// @brief 随机生成数组元素
inline void randomInitArr(vector<int>& arr, int size) {
